...
// --- Scout concept images (optional) ----------------------------------------------

// GAME-ADAPT: opt-in AI concept art for scout drops, generated by an
// image-generator MCP server (e.g. Nano Banana) attached to the drop
// routine's session — docs/scout-routine.md STEP 4b. OFF by default: with
...
export const SCOUT_IMAGES = Object.freeze({
  ...
  // Which image-generation MCP the routine should look for in its session
  // (a hint, not an API contract — any MCP that returns an image works).
  generator: 'nanobanana',

  // Where generated images are committed, repo-relative, trailing slash.
  // Files are named after the pitch: <asset_dir><pitch-id>.<png|jpg|jpeg|webp|svg>.
  asset_dir: 'assets/scout-art/',

  // The templatized image prompt. scout.js buildImagePrompt() fills the
  // {placeholders} per pitch: {seed_a} and {seed_b} are the pitch's two
  // inspiration seeds, {slot} its cosmetic category, plus {title}, {tags},
  // {description}, and the context vars {game_name} / {visual_direction}.
  // {seed_a}, {seed_b}, and {slot} are REQUIRED in the template (checked by
  // scripts/validate-config.mjs) — the fusion-of-two-seeds-per-category
  // premise must survive into the image, and the drop validator re-checks
  // each shipped image_gen.prompt cites them.
  prompt_template:
    'Concept art for "{title}", a {slot} cosmetic for {game_name}, a video ' +
    'game. Fuse two real-world references: {seed_a} and {seed_b}. ' +
    'Mood: {tags}. {description} Art direction: {visual_direction} ' +
    'Single subject, plain dark studio backdrop, no text, no logos, ' +
    'no watermark.',
});
