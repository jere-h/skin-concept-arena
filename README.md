...
Optionally, drops can ship **AI concept images** generated by an
image-generator MCP (e.g. Nano Banana) attached to the routine's session —
off by default, double-gated, with a templatized per-pitch prompt built from
each concept's two seeds and its cosmetic slot, and validator-enforced
provenance. Runbook: `docs/image-generator-mcp-integration.md`.
